  ...
  methods: {
    ...
    saveNode(pageData) {
      ...
      if (this.editMode) {
        ...
        this.$siteNavigationService.createNode(this.navigationNode.id, previousNodeId, this.nodeLabel, this.nodeId, this.visible, this.isScheduled, startScheduleDate, endScheduleDate, pageData?.pageRef, pageData?.pageRef && pageData?.nodeTarget || 'SAME_TAB', nodeLabels)
          .then(() => {
            this.openTargetPage(pageData);
            this.$root.$emit('refresh-navigation-nodes');
          })
